...
func (s *Server) initLogic() *mpl.LogicData {
	...
	salt, err := s.data.GetCfgString("PassSalt", "")
	logicData.PasswordSalt = salt
	if err != nil || logicData.PasswordSalt == "" {
		logicData.PasswordSalt = logicData.getCryptRandKey(32)
		s.data.SetCfgString("PassSalt", logicData.PasswordSalt)
	}
